...
Parameter::Parameter(vector<string> modifiers, Type type, Token name)
    : Parameter{std::move(modifiers), std::move(type), std::move(name), nullptr}
{

}

Parameter::Parameter(vector<string> modifiers, Type type, Token name, ExprPtr expr)
...
Parameter Parameter::instantiate_templated_types(const Type& template_type) const
{
    ...
    return Parameter{modifiers_, std::move(type), name_, std::move(expr)};
}
