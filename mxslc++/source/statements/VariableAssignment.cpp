...
VariableAssignment::VariableAssignment(const Runtime& runtime, ExprPtr lhs, ExprPtr rhs)
    : Statement{runtime}, lhs_{std::move(lhs)}, rhs_{std::move(rhs)}
{
...
void VariableAssignment::execute() const
{
    lhs_->init();
    rhs_->init(lhs_->type());
