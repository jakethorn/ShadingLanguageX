...
28. [Default Expression](#default-expression)
29. [Standard Library](#standard-library)
30. [Scope](#scope)
31. [Preprocessor Directives](#preprocessor-directives)
...
struct in C++/Python (see the [User Guide](https://github.com/jakethorn/ShadingLanguageX/blob/main/docs/UserGuide.md) for more details). 
For example:
```
inline void main()
{
...
[User Guide](https://github.com/jakethorn/ShadingLanguageX/blob/main/docs/UserGuide.md) for more information). For example:
...
`nodedef` functions are the default representation and will create a NodeDef and NodeGraph elements when compiled.
As this is the default, you don't need to add any modifiers to enable this behaviour, but you can add the `nodedef` modifer
...
Default expressions evaluate to the default value of the inferred or specified type. For example:

```
float f = default;      // f = 0.0 (float type inferred)
bool b = default(bool); // b = false (bool type specified)

T foo<bool, float, color3, filename, volumeshader>()
{
    return default(T);
}
```
...
have access to the MaterialX libraries for that version, otherwise it will fail. See the [User Guide](https://github.com/jakethorn/ShadingLanguageX/blob/main/docs/UserGuide.md) for more information.
...
1. Any additional directories passed to the compiler (see the [User Guide](https://github.com/jakethorn/ShadingLanguageX/blob/main/docs/UserGuide.md))
...
the compiler either as command line arguments or using the C++/Python APIs. See the [User Guide](https://github.com/jakethorn/ShadingLanguageX/blob/main/docs/UserGuide.md) for more information.
